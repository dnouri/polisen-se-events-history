...
## Phase 3 release metrics

The release metrics are generated by `scripts/geography_quality_metrics.py` from the already-exported v2 Parquet artifact, not from source JSON or a separate profile input. Regenerate locally with:
...
- `geography-quality.md` — human-readable report with the same headline metrics, ancillary source signal counts, compact month/event-type breakdowns, and representative examples.

The metrics taxonomy remains release/report-level only. `geography_shape_counts` describes the exported null-shape, `assignment_rule_counts` explains the resolver rule, and `source_conflict_counts` reports API/title disagreements. `source_signal_counts` is ancillary API-location/title-suffix granularity context (for example, `api_county__title_municipality`), not another per-row export status. These are intentionally not added as per-row confidence/source/status columns in `events.parquet`.
