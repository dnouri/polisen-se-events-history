...
> **v2 release timing:** The v2 geography schema documented here is implemented in `main`. The public `data-latest/events.parquet` asset is regenerated by the daily release workflow, so it may still expose the previous schema until the next successful release run after this change. The new `geography-quality.json` and `geography-quality.md` metrics assets are created by that same workflow and may 404 until the first successful post-change release run. Run `DESCRIBE` (or generate locally) to confirm before querying v2-only columns.
...
- **Geography quality metrics:** daily releases attach `geography-quality.json` and `geography-quality.md`, generated from the same v2 Parquet artifact after schema validation. These assets may not exist (404) until the first successful release run after the metrics change. Metrics separate exported geography shape counts, deterministic assignment-rule counts, and source conflict counts; `source_signal_counts` is ancillary API/title granularity context.
...
The geography-quality assets may return 404 until the first successful release run after the metrics change.
...
Metrics checks run by default and fail empty artifacts, invalid datetimes, schema/reference/contract/arithmetic inconsistencies, but not real unresolved/conflict rows. Use `--no-check` only for exploratory output when you intentionally want to skip those release-safety checks.
